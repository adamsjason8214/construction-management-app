...
CREATE TABLE tasks (
  ...
  depends_on UUID REFERENCES tasks(id), -- Task dependency
  location TEXT, -- Specific location within construction site
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE project_files (
  ...
  metadata JSONB -- {location, tags, notes}
);
...
CREATE POLICY "Project members can create tasks"
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE project_members.project_id = tasks.project_id
      AND project_members.user_id = auth.uid()
      AND project_members.role IN ('owner', 'manager', 'contractor')
    )
  );
...
-- Daily Logs Policies
CREATE POLICY "Users can view logs in their projects"
...
CREATE POLICY "Contractors and managers can create logs"
...
CREATE POLICY "Log creators and managers can update logs"
...
CREATE POLICY "File uploaders and managers can delete files"
...
-- Trigger to call handle_new_user when user signs up
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- Function to update project actual_cost when tasks are completed
CREATE OR REPLACE FUNCTION update_project_cost()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE projects
    SET actual_cost = actual_cost + (NEW.actual_hours * 50) -- $50/hour default rate
...
-- Create a sample admin user (update with your actual auth user ID)
-- INSERT INTO profiles (id, email, full_name, role, company)
-- VALUES (
